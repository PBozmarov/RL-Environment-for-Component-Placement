"""
Dummy placement environment for rectangular components with pins.
"""
...
class Pin(object):
    ...
    def update_pin_position(self, component_x: int, component_y: int):
        """Update the position of the pin as the component is placed.

            Updates the absolute position of the pin based on the
            position of the top-left corner of the component that the pin is placed on.
    ...
    def calculate_feature(self) -> Tuple[np.ndarray, np.ndarray]:
        """Calculates the feature vector for the pin.

            This method calculates both the numerical and categorical feature vectors
            for the pin. The numerical feature vector contains:
                - the relative x position of the pin
                - the relative y position of the pin
                - the absolute x position of the pin
                - the absolute y position of the pin
            The categorical feature vector contains:
                - the net id of the pin

        Returns:
            Tuple[np.ndarray, np.ndarray] : the numerical feature vector for the pin,
                the categorical feature vector for the pin.
        """
    ...
    def __repr__(self) -> str:
        """Calls the __str__ method to return a string representation of the pin.
...
class Component(object):
    ...
    def place_component(self, orientation: int, x: int, y: int):
        ...
        # Update the relative and absolute positions of the pins based on orientation
...
class DummyPlacementEnv(gym.Env):
    ...
    Attributes:
        ...
        max_wirelength (float): The maximum wirelength possible to use for the wirelength
    ...
    def __init__(  # noqa: max-complexity: 15
        ...
        # Clip net_distribution and pin_spread to [0, 9]
        ...
        self.reward_intersection = (
            -1
        )  # dummy value to be updated after reward is calculated
        self.reward_wirelength = -1.0
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "all_components_feature": gym.spaces.Box(
                    ...
                    shape=(self.max_num_components, 5),
                    dtype=np.float64,
                ),
                "all_pins_num_feature": gym.spaces.Box(
                    ...
                    shape=(
                        ...
                        self.max_num_pins_per_component,
                        4,
                    ),
                    dtype=np.float64,
                ),
        ...
        self.all_pins_num_feature: np.ndarray = np.zeros(
            (self.max_num_components, self.max_num_pins_per_component, 4)
        )
    ...
    def validate_env_params(  # noqa: max-complexity: 13
        self,
        height: int,
        width: int,
        max_component_w: int,
        ...
        min_num_components: int,
        max_num_components: int,
        min_num_pins_per_net: int,
        max_num_pins_per_net: int,
        min_num_nets: int,
        reward_beam_width: int,
        reward_type: str,
    ):
        """Validate the environment parameters.
        ...
        Raises:
            ValueError: - if the grid size is less than or equal to 0.
                        ...
                        - if the reward type is not one of "beam", "centroid", or
                            "both".
        """
        # Check if the grid size is less than or equal to 0
        # and if the component size is greater than the grid size
        if height < 0 or width < 0:
            raise ValueError("Grid size must be greater than 0.")
        if max_component_w > width or max_component_h > height:
            raise ValueError(
                "Component size must be less than or equal to the grid size."
            )

        # Check if the component size is less than or equal to 0
        # and if the maximum number of components is greater than the area of the grid
        # or less than or equal to 0
        if min_component_w < 1 or min_component_h < 1:
            raise ValueError("Component size must be greater than 0.")
        if max_num_components < 1 or max_num_components > height * width:
            raise ValueError(
                "Number of components must be greater than 0"
                + "and less than or equal to the grid area."
            )

        # Check if the min_num_pins_per_net is greater than the max_num_pins_per_net and
        # greater than 2
        if min_num_pins_per_net > max_num_pins_per_net:
            raise ValueError(
                "min_num_pins_per_net must not be greater than max num pins per net"
            )
        if min_num_pins_per_net < 2:
            raise ValueError("min num pins per net must be less than or equal to 2.")

        # Check that min_num_pins_per_net * min_num_nets is less than or equal to the
        # total minimum area covered by the components
        if (
            min_num_pins_per_net * min_num_nets
            > min_component_w * min_component_h * min_num_components
        ):
            raise ValueError(
                "min_num_pins_per_net * min_num_nets must be less than or equal to the total minimum area covered by the components"
            )

        # Validate the beam with and reward type (must be positive integer)
        if reward_beam_width < 1 or not isinstance(reward_beam_width, int):
            raise ValueError("Beam width must be a positive integer.")
        if reward_type not in ["beam", "centroid", "both"]:
            raise ValueError(
                "Reward type must be either 'beam', 'centroid', or 'both'."
            )
    ...
    def is_intersect(
        ...
        Note:
            An intersections here is counted only if it is inside the grid.
        """
        x1, y1 = line1[0]
        x2, y2 = line1[1]
        x3, y3 = line2[0]
    ...
    def calculate_upper_bound_intersections(self) -> int:
        ...
        Returns:
            int: Upper bound on number of intersections.
        """
        max_num_intersections = int(
            (
                0.5
    ...
    def find_reward(self) -> float:
        ...
        # Find the average number of pins using the average component size and
        # the average number of components
        ...
        # Find the average number of pins using the average number of pins per net
        # and the average number of nets
        ...
        if self.reward_type == "beam":
            if self.reward_beam_width == 0:
                raise ValueError("Beam search is used but beam_width is 0.")
            else:
    ...
    def allocate_pins_to_components(self):
        ...
        self.num_components_w_pins = min(
            max(int(((self.pin_spread + 1) / 10) * self.num_components), 1),
            self.num_components,
        )
    ...
    def allocate_pins_to_components_for_net(  # noqa: max-complexity: 13
        self,
        net: int,
        ...
        while num_pins_unassigned > 0:
            ...
            for component, num_pins in zip(
                ...
                for pin in range(num_pins):
                    self.net_pins[net][pin + pin_id].pin_id = pin
    ...
    def update_all_components_feature(self, components: List[Component]):
        ...
        for component in components:
            feature = component.calculate_feature(self.area)
    ...
    def update_all_pins_feature(self, components: List[Component]):
        ...
        for component in components:
            for pin in component.pins:
                # Calculate pin features
                pin_num_feature, pin_cat_feature = pin.calculate_feature()

                # Update all_pins_num_feature and all_pins_cat_feature
                self.all_pins_num_feature[
                    pin.component_id, pin.pin_id, :
                ] = pin_num_feature
                self.all_pins_cat_feature[
                    pin.component_id, pin.pin_id, :
                ] = pin_cat_feature
    ...
    def step(  # noqa: max-complexity: 15
        ...
        if valid_action:
            ...
            comp_feature = self.current_component.calculate_feature(self.area)
            self.all_components_feature[
                self.current_component.comp_id, :
            ] = comp_feature

            # Update the all pins feature arrays
            for pin in self.current_component.pins:
                pin_num_feature, pin_cat_feature = pin.calculate_feature()
                self.all_pins_num_feature[
                    pin.component_id, pin.pin_id, :
                ] = pin_num_feature
            ...
            if verbose:
                print_string = "Grid:\n{}\n\nAction mask:\n{}\n\nReward: {}\n\n"
                print(print_string.format(self.grid, self.action_mask, reward))
    ...
    def rows_cols_to_mask(
        ...
        #  Get the rows and columns to mask
        rows_to_mask = [
            i
    ...
    def compute_action_mask_orientation(
        ...
        # Mask rows and columns that are already occupied
